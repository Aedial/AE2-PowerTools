...
public enum MaintainerState {
     ...
     */
    DISABLED(0x40303040, 0x808080, TextFormatting.DARK_GRAY),

    /**
     * Recipe has finished running and is waiting for the next scheduled run.
     */
    IDLE(0x00000000, 0x707070, TextFormatting.GRAY),

    /**
     * Task is scheduled to run (waiting for its time slot).
     */
    SCHEDULED(0x4080C0FF, 0x50A0D0, TextFormatting.AQUA),

    /**
     * Task is actively running (crafting in progress).
     */
    RUNNING(0x4040FF40, 0x30B030, TextFormatting.GREEN),

    /**
     * Task is running but stalled (e.g., waiting for resources).
     */
    STALLED(0x40FFFF00, 0xB0B000, TextFormatting.YELLOW),

    /**
     * Task could not be run (missing recipe, no free CPU, missing resources).
     */
    ERROR(0x40FF4040, 0xD03030, TextFormatting.RED),

    /**
     * Post-crafting issue (no space for output).
     */
    POST_ERROR(0x40C040FF, 0xA030D0, TextFormatting.DARK_PURPLE);

    private final int backgroundColor;
    private final int textColor;
    private final TextFormatting colorCode;

    MaintainerState(int backgroundColor, int textColor, TextFormatting colorCode) {
    ...
    public TextFormatting getColorCode() {
        return colorCode;
    }
