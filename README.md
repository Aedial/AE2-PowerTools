...
- **Performance Optimization**: Caches recipe simulation on load and only recalculates it when the recipe changes. This means the expensive part (when it comes to crafting) is only done once, and then the AutoCrafter will just rely on the cached results for subsequent crafts, which is very fast.
